...
- `scripts/agent_conclave.py`: multiple isolated repo-aware specialists launched concurrently, followed by prompt-only synthesis. The machine-wide g4f pool controls how many calls are active; excess roles wait in FIFO order.
...
7. Repo-aware agent calls default to a 900-second request timeout plus a separate 3600-second queue timeout because ChatGPT may make several MCP reads and independent sessions may be waiting for a worker lease. Keep the command in the foreground and let the open request wait; do not repeatedly poll ChatGPT.
...
Prompt-only `conclave.py` remains serial by default for compatibility. Repo-aware `agent_conclave.py` is parallel by default and caps itself at two workers, matching the supervised two-worker g4f pool; use `--serial` when deliberate role ordering is preferable.
