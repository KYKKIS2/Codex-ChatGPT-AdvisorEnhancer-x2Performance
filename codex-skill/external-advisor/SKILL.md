...
Pro Extended is intended for hard advisor questions, architecture reviews, high-risk debugging, and important strategic decisions, not routine checks. It can run long prompts silently for several minutes before returning. When neither `--timeout` nor `ADVISOR_TIMEOUT` is supplied, every prompt-only wrapper automatically uses timeout `0`: prompt acceptance stays bounded, then the foreground process waits for the actual final turn. Explicit positive timeouts remain deliberate operator deadlines. Router, conclave, verifier, before-final critique, and evaluation subprocesses preserve timeout `0` instead of converting it into a short local kill deadline. Prompt-only conclave also does not launch a useless synthesizer when every specialist failed. If a long Pro Extended call appears to fail but a tiny `OK` diagnostic works, do not assume the model cannot handle the prompt. First inspect the actual timeout/transport error; rerun only when the turn journal proves another submission is safe.
