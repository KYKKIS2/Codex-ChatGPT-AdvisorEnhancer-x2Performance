...
For ChatGPT web Intelligence/thinking choices, set the private ChatGPT field explicitly with `ADVISOR_THINKING_EFFORT` or `--thinking-effort`. Examples: `extended`, `max`, `pro-extended`, `extra-high`, `high`, `medium`, or `none`. This is separate from `ADVISOR_REASONING_EFFORT`; the advisor maps these values to ChatGPT's private web `thinking_effort` field and defaults normal advisor calls to `extended`.

Current ChatGPT private effort values are `min`, `standard`, `extended`, and `max`. The advisor maps friendly names to those values: `low`/`light` -> `min`, `medium` -> `standard`, `high` -> `extended`, and `extra-high`/`xhigh`/`heavy` -> `max`. Do not send stale raw values such as `high` or `xhigh` directly to g4f; ChatGPT can reject them as `Invalid conversation body`. Unknown values fail locally unless `ADVISOR_ALLOW_UNKNOWN_THINKING_EFFORT=true` is set for diagnostics. When no private effort is set, `advisor.py` defaults to `extended`, because that is the currently safe Thinking-lane advisor route.

Pro Extended should select both the request model slug and the effort field. Use `ADVISOR_THINKING_EFFORT=pro-extended`; the scripts automatically request `gpt-5-5-pro` and send `thinking_effort=extended`. If a normal default model such as `gpt-5-5-thinking` is also set, the scripts override it to `gpt-5-5-pro` to avoid silent downgrades. Override the automatic Pro Extended request model with `ADVISOR_PRO_EXTENDED_MODEL` if ChatGPT changes the private slug; set `ADVISOR_ALLOW_PRO_MODEL_OVERRIDE=true` only for deliberate diagnostics.

`gpt-5-5-thinking` is the normal advisor default, paired with `thinking_effort=extended`. The unsafe route is `gpt-5-5-thinking` without private effort, `min`, or `standard`, which current ChatGPT metadata can resolve to `gpt-5-3-mini`; if no thinking is explicitly requested, the scripts rewrite that route to plain `gpt-5-5` unless `ADVISOR_ALLOW_LEGACY_THINKING_MODEL=true` is set for diagnostics. `extended` or `max` should be used when a Thinking-lane run is desired. The Pro Extended path requests/defaults `gpt-5-5-pro` with `thinking_effort: extended`. To request that path from Codex, use `ADVISOR_THINKING_EFFORT=pro-extended` rather than bare `extended`. This repo's setup patch adds the required g4f/OpenaiChat conversation-turn WebSocket handoff path for extended turns. If an extended call returns empty text, refresh the HAR/session first, then inspect the WebSocket handoff in `OpenaiChat`.
...
For Pro Extended, `advisor.py` also checks synced ChatGPT metadata after the response. A valid current browser Pro Extended turn can report `model_slug`/`default_model_slug: gpt-5-5-pro` and `thinking_effort: extended` while still showing `resolved_model_slug: gpt-5-3-mini`; do not treat that resolved field alone as a downgrade. For Pro requests, verify the Pro request fields plus `thinking_effort: extended`. For non-Pro persistent ChatGPT-backed advisor calls, `advisor.py` rejects known downgraded resolved models by default. Currently `ADVISOR_REJECT_RESOLVED_MODEL_SLUGS` defaults to `gpt-5-3-mini`. Set `ADVISOR_ALLOW_RESOLVED_MODEL_DOWNGRADE=true` only for deliberate transport diagnostics.
