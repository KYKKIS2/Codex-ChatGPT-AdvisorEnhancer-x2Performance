...
- Every prompt-only advisor call defaults to timeout `0` when neither `--timeout` nor `ADVISOR_TIMEOUT` is supplied, including normal `max` and Pro Extended. The router, conclave, verifier, critique, and evaluation wrappers must preserve that unlimited completion wait through every subprocess layer; an explicit positive value is the only normal way to impose an operator deadline.
...
- Prompt words such as `token`, `auth`, `privacy`, or `security` are ordinary topic text and must not trigger a special automatic safety or security lane. Route from the task shape, or force a security-specialist mode explicitly when it is actually wanted.
